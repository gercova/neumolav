$(document).ready(function(){
    ...
    /*function sessionCurrent(){
        fetch(`${API_BASE_URL}secury/usuarios/sessionCurrent`, {
            method: 'GET',
            headers: {
        ...
        .then(res => res.json())
        .then(res => {
            $('#profile-img').attr('src', `${API_BASE_URL + res.user.profilePhoto}`)
    ...
    /*function cargarVista(nombreVista) {
        ...
        $.ajax({
            ...
            error: function(xhr, status, error) {
                $('#app').html(`<p style="color: red;">Error al cargar la vista: ${xhr.status} ${xhr.statusText}</p>`);
            }
        });
    }*/

    /*async function cargarpagina(url, forzar = false) {
        if (event) {
            event.preventDefault(); // Prevenir el comportamiento predeterminado del evento si se proporciona
        }
        
        if (X_FLG_SUSPENDIDO == true && forzar == false) {
            url = ''; //'https://cpe.facturaperuana.com/suspendido?url=' + url;
            url = '';//'https://cpe.facturaperuana.com//panel/redirigir_pagina?url=' + url;
            var nuevaURL = url;
            window.location.href = nuevaURL;
            return false;
        };
    
        const divCapaPagina = document.querySelector("#div_capa_pagina");
        mostrarAnimacionCarga(divCapaPagina);
    
        try {
            const timeoutId = setTimeout(() => {
                mostrarMensajeError(divCapaPagina, "La página está tardando más de lo normal en cargar. Por favor, espere un momento más.");
            }, 10000);
            ...
            $('#div_capa_pagina').html(html);
        } catch (error) {
            mostrarMensajeError(divCapaPagina, `${error.message}`);
        }
    }*/
